...
public class GenerateExpensesReportPdfUseCase : IGenerateExpensesReportPdfUseCase
{
    ...
    public async Task<byte[]> Execute(DateOnly month)
    {
        ...
        var document = CreateDocument(logedUser.Name, month);
        ...
        {
            var table = CreateExepenseTable(page);
            ...
            AddAmountExpense(row.Cells[3], expense.Amount);
    ...
    private void CreateHeader(Section page)
    {
        ...
        var row = table.AddRow();
        row.Cells[0].AddParagraph(ResourceReportGenerationMessages.HELLO);
        row.Cells[0].Format.Font = new Font { Name = FontHelper.RALEWAY_BLACK, Size = 28 };
        row.Cells[0].VerticalAlignment = MigraDoc.DocumentObjectModel.Tables.VerticalAlignment.Center;

        var assembly = Assembly.GetExecutingAssembly();
        var directoryName = Path.GetDirectoryName(assembly.Location);
        var pathFile = Path.Combine(directoryName!, "Logo", "hello.png");

        var img = row.Cells[1].AddImage(pathFile);
        img.Width = "9mm";
        img.Height = "9mm";
    }
    ...
    private Table CreateExepenseTable(Section page)
    {
        var table = page.AddTable();
    ...
    private void AddAmountExpense(Cell cell, decimal amount)
    {
